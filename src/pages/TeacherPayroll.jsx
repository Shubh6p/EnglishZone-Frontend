...
import { Landmark, ArrowDownToLine, CheckCircle2, AlertCircle, Sparkles } from 'lucide-react';
import './TeacherPayroll.css';

const TeacherPayroll = () => {
  const { teacherSalaryData } = useContext(AppContext);
  const [selectedMonth, setSelectedMonth] = useState('June');

  const payroll = teacherSalaryData[selectedMonth] || {
    gross: 0,
  ...
  const handleDownload = () => {
    alert(`PDF Payslip for Mrs. Anjali Sen (${selectedMonth} 2026) has been generated and downloaded successfully.`);
  };
